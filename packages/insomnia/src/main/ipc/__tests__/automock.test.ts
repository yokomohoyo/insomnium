...
describe('generateRequestTemplate', () => {
  ...
  it('emits empty/zero defaults for scalars', () => {
    ...
    `);
    expect(generateRequestTemplate(s, 'M')).toStrictEqual({
      s: '', i: 0, d: 0, b: false, by: '',
    });
  });

  it('emits empty arrays for repeated fields (not one-element)', () => {
    const s = svc(`
      syntax = "proto3";
  ...
  it('uses the first option for oneof groups', () => {
    const s = svc(`
      syntax = "proto3";
      message Req {
        oneof which { string first = 1; int32 second = 2; }
      }
      message Res {}
      service S { rpc M (Req) returns (Res); }
    `);
    // oneof field with a scalar resolves to the field's value, not nested.
    const t = generateRequestTemplate(s, 'M') as Record<string, unknown>;
    // protobufjs treats oneof fields as siblings; first option wins.
  ...
  it('uses the first enum value', () => {
    const s = svc(`
      syntax = "proto3";
      enum E { E_UNSPECIFIED = 0; E_ONE = 1; E_TWO = 2; }
      message Req { E e = 1; }
      message Res {}
      service S { rpc M (Req) returns (Res); }
    `);
  ...
  it('does not infinite-loop on recursive types', () => {
    ...
    `);
    // Should complete without exceeding stack depth.
