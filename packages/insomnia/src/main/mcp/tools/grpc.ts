...
export function registerGrpcTools(server: McpServer) {
  server.tool(
    ...
    async ({ requestId, environmentId, protoMethodName, timeoutMs, maxResponses, messages }) => {
      ...
      if (!method) {
        return { content: [{ type: 'text', text: JSON.stringify({ error: `method ${rendered.protoMethodName} not found` }) }], isError: true };
      }
