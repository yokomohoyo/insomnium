...
export async function fetchProto(url: string): Promise<FetchedProto> {
  ...
  switch (kind.kind) {
    case 'github-blob':   return fetchGithubBlob(input);
    case 'github-tree':   return fetchGithubTree(input);
    case 'github-raw':    return fetchHttpsSingle(input);
    case 'bsr':           return fetchBsr(input);
...
async function fetchGithubBlob(url: string): Promise<FetchedProto> {
  const raw = githubBlobToRawUrl(url);
  const file = await fetchHttpsText(raw);
...
async function fetchGithubTree(url: string): Promise<FetchedProto> {
  ...
  await walkGithubDir(owner, repo, ref, subPath, '', files);
  if (files.length === 0) {
...
async function walkGithubDir(
  ...
  const entries = await fetchJson<GhContentEntry[]>(api);
  for (const entry of entries) {
    if (entry.type === 'dir') {
      await walkGithubDir(owner, repo, ref, entry.path, joinPath(outPath, entry.name), out);
    } else if (entry.type === 'file' && entry.name.endsWith(PROTO_EXT) && entry.download_url) {
      const protoText = await fetchHttpsText(entry.download_url);
...
async function fetchHttpsSingle(url: string): Promise<FetchedProto> {
  if (!url.endsWith(PROTO_EXT)) {
    throw new Error(`Expected a URL ending in .proto, got: ${url}`);
  }
  const protoText = await fetchHttpsText(url);
  const name = url.split('/').pop()!;
...
async function fetchBsr(url: string): Promise<FetchedProto> {
  ...
  // as a single bundle. Public modules need no auth.
